...
public class PackLogin {
    ...
    public byte[] packPart2Core(com.ruoyi.im.entity.Tlv t){
        pack.empty();
...
//                 (byte[]) t.getTlv_t0106().get("hexMd5UserPass")));
        pack.setBin(tlv.tlv_t0106((byte[]) t.getTlv_t0106().get("tlv0106Token")));



        //int mMiscBitmap,int mSubSigMap,int sub_appid_list
        pack.setBin(tlv.tlv_t0116((String) t.getTlv_t0116().get("mMiscBitmap"), (String) t.getTlv_t0116().get("mSubSigMap")));
        ...
        pack.setBin(tlv.tlv_t016A((byte[]) t.getTlv_t016A().get("tlv016AToken")));
...
//        if(part1Token==null){//记录返回的getPart1Token
            pack.setHex("00 00 00 08 64 AE 39 1E");
//        }else {
...
//        }
        ...
        pack.setBin(ksId);
        pack.setShort((short)(imsi.length+appVersion.length+4));//中国移动用户标识码 +apk版本带A
        String su="7C"+ ByteToAll.byteToHxe(imsi)+"7C"+ ByteToAll.byteToHxe(appVersion);
    ...
    public UnPackLogin unPackLogin(UnPackLogin unPackLogin){//理想化解析
        ...
        //part2Core
        unPack.setData(teaService.decrypt(unPack.getAll(), unPackLogin.getShare_key()));
        unPack.getShort();//9
        ...
        if(Arrays.equals(loginState,new byte[3])){//正常登录
            ...
            unPack.setData(teaService.decrypt(tlv_v0119, unPackLogin.getTgt_key()));
